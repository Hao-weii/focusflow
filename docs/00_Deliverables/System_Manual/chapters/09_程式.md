...
清單中的「使用個案」欄以第 5 章表5-1-1 的 UC-01～UC-09 為準：UC-01 註冊與登入、UC-02 課程與修課名單管理、UC-03 影片加入與處理追蹤、UC-04 網頁觀看與課程問答、UC-05 LINE 綁定與提問、UC-06 個人資料與通知、UC-07 短影音瀏覽、UC-08 短影音腳本與成品審核、UC-09 系統維運。「共用」表示元件被多數使用個案共同依賴（例如認證與統一回應格式）；「—」表示元件屬於啟動、維運或評估用途，不直接對應使用個案。
...
| B-RT-14 | `admin.routes.js` | 管理員統計、使用者、影片、事件、系統狀態、全站公告與問題回報處理；整個 router 先驗管理員角色。 | UC-09 |
| B-RT-15 | `feedback.routes.js` | 已登入使用者送出問題回報（最多 3 張截圖）與讀取附件。 | —（見 9-2-4） |
...
| B-CT-10 | `notification.controller.js` | 解析並限制通知查詢參數，處理單筆與全部已讀、全站公告。 | UC-06、UC-09 |
...
| B-CT-15 | `feedback.controller.js` | 問題回報的建立與附件讀取。 | —（見 9-2-4） |
...
| B-SV-18 | `feedback.service.js` | 問題回報建立、管理員列表與狀態更新、附件讀取權限檢查。 | —（見 9-2-4） |
...
| B-SV-28 | `notification.service.js` | 站內通知列表、已讀、全站公告，以及影片處理完成與短影音退回通知。 | UC-06、UC-03、UC-09 |
...
| B-MD-20 | `feedback.model.js` | 問題回報：回報者、角色、分類、嚴重程度、描述、附件摘要與處理狀態。 | —（見 9-2-4） |
| B-MD-21 | `feedbackAttachment.model.js` | 問題回報截圖本體，與回報主文件分離。 | —（見 9-2-4） |
...
| B-MW-07 | `feedbackUpload.middleware.js` | 問題回報附件：最多 3 張、每張 10 MiB。 | —（見 9-2-4） |
...
| F-CP-10 | `components/IssueReportLauncher.jsx` | 學生與教師的問題回報視窗：分類、嚴重程度、描述與最多 3 張截圖，送至 `/feedback`；另附外部意見表單連結。 | —（見 9-2-4） |
...
| --- | --- | --- | --- | --- |
| 表9-1-14 | 認證與角色授權 middleware | B-MW-01、B-MW-02 | 共用（UC-01～UC-09） | — |
...
| 輸入 | 使用者、分頁參數；管理員公告內容；完成處理的影片 |
| 輸出 | 通知列表與未讀數、已讀結果；公告與影片完成通知 |
| 責任 | 提供通知列表（有上限的分頁）、單筆與全部已讀；影片處理完成時，通知掛載該影片之課程中具有效修課且帳號啟用的學生，同一學生只通知一次；管理員可發送全站公告；短影音成品被退回時通知教師。 |
...
- **問題回報尚未列入第 5 章使用個案。**`feedback` 相關元件（B-RT-15、B-CT-15、B-SV-18、B-MD-20～21、B-MW-07、F-CP-10）提供學生與教師回報問題、管理員於 F-PG-14 處理；第 5 章 UC-01～UC-09 未涵蓋送出回報的流程，第 8 章亦未列 `feedbacks`／`feedbackattachments` 集合，是否補入由團隊確認。
